...
import { ClientFloatingWrapperLuxury } from "@/ui/components/nav/components/luxury-goods/FloatButton/FloatingWrapper";
import {
		...
		if (activeTab === "myposts" && user && !isLoading) {
			if (!fetchedMyPosts) {
				...
				void executeGraphQL<ProductListByCategoryByUserQuery, ProductListByCategoryByUserQueryVariables>(
					ProductListByCategoryByUserDocument,
					{ variables: { slug: LUXURY_GOODS_CATEGORY_SLUG, userId } },
				)
					.then(({ category }) => {
						const nodes =
							category?.products?.edges
